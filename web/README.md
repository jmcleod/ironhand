...
- Optionally configure TOTP 2FA from the dashboard by scanning the QR code (or entering the secret manually), then provide the one-time code on future logins.
...
Note: the secret key is shown once at registration and cannot be retrieved from the API later.
