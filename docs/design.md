...
Experimental MPC DKG is orchestrated as a transaction-shaped signer ceremony. The API assigns each ceremony a `dkg_session_id`, prevalidates the active signer count against the requested threshold, starts and finalizes signers through authenticated internal calls, aborts touched signers on failure, and commits signer state only after the vault key record and encrypted fragments have been stored.

MPC signing approvals are session-scoped capabilities. A signer approval covers the vault ID, key ID, session ID, threshold, full participant set, message hash, and expiry; completion then accepts any valid threshold-sized approval subset before the session expires.
